function [results] = minimization_synthetic(options, n_tests)

rng(0)

bounds = [50 * 1e-3, 50 * 1e-3, 50 * 1e-3, 20 * pi / 180, 20 * pi / 180, 20 * pi / 180];
nvars = 6;
positions_angles = 2 * (rand(n_tests,6) - 0.5) .* bounds;
points_positions = operators.kamera(positions_angles) + randn(n_tests, 10)*5e-3;
...
for i = 1:datasets
    ...
    for j = 1:repeats
        ...
        formating.ProgressBar(i, datasets)
    end
